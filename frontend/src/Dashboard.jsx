...
export default function Dashboard({ account, startDate, endDate }){
  ...
  // Load when account changes or when user explicitly applies filters
  ...
  if (dates.length === 0) return <div>No data available for chart.</div>

  // Sum totals per category across dates
  const totals = {}
  dates.forEach(d => {
  ...
  // Prepare pie data (Top N) but don't return early — render controls above
  ...
  const datasets = catsToShow.map((cat, idx) => {
    ...
    return {
      ...
      tension: 0.3,
      borderWidth: 2,
      // make point visible; small markers for multi-date, larger for single-date
      ...
      pointStyle: 'circle',
      hoverRadius: 6,
      // for single-date line, show a filled area to make the value visible
  ...
  try {
    return (
      <Box>
        ...
        />
        {/* (table control removed) */}
      </Box>
      <Box>
        {chartType === 'pie' ? (
          // Render one pie per date for side-by-side comparison when multiple dates
